...
<head>
    ...
    <title>RESTAURANT - lista zamówień</title>
...
<body>
    ...
    <div id="infoDiv" class="alert alert-<?=$_SESSION['msg_type']?>">
        <?php
    ...
    <main class="mb-5">
        <div class="container">
            ...
            <div class="container-fluid mb-3 mt-3">
                <form action="exportOrders.php" method="POST">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <table id="ordersTable" class="table table-bordered display nowrap" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Imie</th>
                    ...
                    <div class="modal-content">
                        <form action="delete.php" method="POST">
                            <div class="modal-body">
                                <h4 id="komunikat"></h4>
                                <input type="hidden" name="id_zamowienia" id="id_zamowienia" />
                            </div>
                            <div class="modal-footer">
                                <button type="button" id="close" class="btn btn-secondary"
                                    data-dismiss="modal">Zamknij</button>
                                <button type="submit" name="delete_student" class="btn btn-danger">Usuń</button>
                            </div>
                        </form>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    $('#ordersTable').DataTable({
        ...
        'ajax': {
            ...
            'complete': function() //po załadowaniu danych AJAXEM
            {
                //INTERWAŁ BO JAK TABELA RESPONSYWNA
                setInterval(function() {
                    $('.delete_btn').click(function(e) { //jeśli klikniesz na  DELETE button
                        e.preventDefault();
                        console.log("klik")
                        var order_id = $(this).closest('tr').find('.id_zamowienia')
                            .val(); //zczytujemy id tego zamówienia z inputa hidden
                        order_id = parseInt(order_id)
                        console.log(order_id)
                        $('#deleteStudentModal').modal('show'); //pokzajemy okno dialogowe
                        $('#id_zamowienia').val(
                            order_id); //ustawiamy wartośc id dla input hidden
                        $('#komunikat').text('Usunąć zamówienie numer ' + order_id + '?');
                    })

                    $('#close').click(function(e) { //jeśli klikniesz na close button
                        e.preventDefault();
